...
# Continuação Data Lakehouse

Data Lakehouse é um conceito que mistura os conceitos de data lake e data warehouse, oferecendo uma arquitetura unificada que permite uma arquitetura de dados de maneira bruta e estruturada, além de oferecer capacidades de processamento e análises avançadas.

## Componentes de um Data Lakehouse 

### Armazenamento unificado

Tem a capacidade de integrar dados estruturados, não estruturados e semi-estruturados em um unico ambiente, seja ele uma ferramenta ou serviço de armazenamento de dados, permitindo a flexibilidade no acesso e analise de dados.

- **Tecnologias:** As seguintes ferramentas são utilizadas na função de armazenamento unificado, sendo elas o Apache Hudi que adiciona funcionalidades de versionamento e atualização em data lakes, possibilitando consultas em tempo real, Delta Lake ela adiciona ACID transactions (Atomicidade, Consistência, Isolamento, Durabilidade) ao armazenamento de dados, permitindo operações seguras e confiáveis em data lakes e também Apache Iceberg que suporta grandes volumes de dados com evolução de esquemas e operações em tabelas, sendo otimizado para consultas em larga escala.

### Camada de processamento e consulta

É o que facilita o processamento e a analise dos dados no data lakehouse, isso pode ocorrer tanto em live ou em batch.

- **Tecnologia:** As seguintes ferramentas são utilizadas para execurar esse trabalho, sendo elas o Apache Spark uma Ferramenta de processamento distribuído que lida com grandes volumes de dados e suporta tanto processamento em batch quanto em tempo real, Presto/Trino um Motor de consulta SQL distribuído que permite executar consultas de alta performance diretamente no data lakehouse e o Apache Flink que é uma Ferramenta de processamento de dados em tempo real que permite análise contínua de fluxos de dados.

### Ferramentas BI e visualização

É o que permite a visualização e a analise dos dados em um lakehouse, assim facilitando a tomada de decisão baseada em dados.

- **Tecnologias:** As ferramentas utilizadas para visualisação e BI são Tableau uma ferramenta de visualização de dados que permite criar dashboards interativos a partir de dados armazenados no lakehouse, Microsoft Power BI uma ferramenta de análise e visualização de dados que se integra com data lakehouses para criar relatórios e dashboards interativos e o Looker uma ferramenta de BI que suporta consultas SQL e se conecta diretamente a data lakehouses para análise de dados.

### Camada de Machine Learning e IA

Essa camada integra ferramentas de aprendizado de maquina e inteligencia artificial, permitindo a construção e implantação de modelos diretamente no data lakehouse

- **Tecnologia:** As tecnologias que são utilizadas em trabalhos envolvendo ML e IA nesse conceito de data lakehouse é o Databricks uma plataforma de análise unificada que combina processamento de dados com machine learning em um ambiente colaborativo, MLflow que é uma plataforma de gerenciamento de ciclo de vida de modelos de machine learning, com suporte a experimentos, deploys e monitoramento e também o Amazon SageMaker um serviço gerenciado para construção, treinamento e implantação de modelos de machine learning diretamente integrado com AWS Data Lakehouse.

### Benefícios do Data Lakehouse

- **Flexibilidade:** Em seu conceito, permite armazenar dados em seu formato bruto, mantendo a capacidade de análise estruturada.

- **Custo-benefício:** É capaz de combinar os baixos custos de armazenamento de data lakes com a eficiência analítica dos data warehouses.

- **Eficiência Analítica:** Suporta consultas de alta performance e análise em tempo real, proporcionando insights rápidos.

- **Simplificação da Arquitetura:** Reduz a necessidade de manter ambientes separados para data lakes e data warehouses.